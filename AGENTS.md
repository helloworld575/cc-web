...
- When diagnosing NAS, provider, cron, deployment, or production-only failures, collect the relevant sanitized logs with `npm run nas:logs` before drawing a conclusion. Use timestamps, service names, request IDs, duration, status, and safe error codes as evidence; do not paste credentials, prompts, authorization headers, or raw upstream bodies into reports.
