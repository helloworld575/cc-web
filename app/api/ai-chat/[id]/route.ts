...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const chat = db.prepare('SELECT * FROM ai_chat_history WHERE id = ?').get(id) as any;
...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params;
  const existing = db.prepare('SELECT * FROM ai_chat_history WHERE id = ?').get(id) as any;
...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params;
  db.prepare('DELETE FROM ai_chat_history WHERE id = ?').run(id);
  return Response.json({ ok: true });
}
