...
describe('PUT /api/ai-providers/[id]', () => {
  ...
  it('does not let an updated database provider override the env-backed default provider', async () => {
    mockSession(true);
    ...
    expect(res.status).toBe(200);
    const updateCall = stmt.run.mock.calls[stmt.run.mock.calls.length - 1];
    expect(updateCall[7]).toBe(0);
  });
});
