...
describe('POST /api/bazi', () => {
  ...
  it('returns 401 without session', async () => {
    mockSession(false);
    const { POST } = await import('@/app/api/bazi/route');
  ...
  it('returns 429 when rate limited', async () => {
    ...
    const { POST } = await import('@/app/api/bazi/route');
    const res = await POST(postReq(
      { year: 1990, month: 1, day: 1, hour: 0, aspect: '事业' },
  ...
  it('returns 400 on missing params', async () => {
    mockSession(true);
    const { POST } = await import('@/app/api/bazi/route');
    const res = await POST(postReq({ year: 1990 }));
    expect(res.status).toBe(400);
  });
  ...
  it('returns SSE stream on success', async () => {
    ...
    const { POST } = await import('@/app/api/bazi/route');
    const res = await POST(postReq(
      { year: 1990, month: 1, day: 1, hour: 0, gender: 'male', aspect: '事业' },
      { 'x-forwarded-for': '1.2.3.4' },
    ));
