...
登录后从 `/admin` 进入个人工作台。工作台聚合待办截止事项、AI Chat 与 Claude Code 最近会话，并提供技能、内容和订阅的分组入口；`/tools?tab=...` 支持直接打开指定工具视图。工作台只保存 cc-web 自己的任务和会话摘要，安全评估仍通过受限 BFF 访问独立部署的 `sec_ai_tool`。
